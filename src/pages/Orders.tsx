  ...
  const { data: orders, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("orders")
        .select("*, profiles:created_by(full_name, email), order_items(qty), entities(name, requires_approval)")
      ...
      <Table>
        ...
        <TableBody>
          {items.map((order, i) => {
            ...
            return (
              <TableRow key={order.id} className={`text-sm animate-fade-in ${isPendingApproval ? "bg-warning/5" : ""}`} style={{ animationDelay: `${i * 50}ms` }}>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {order.status === "approved" && (
                        <DropdownMenuItem onClick={() => updateStatus.mutate({ id: order.id, status: "fulfilled" })}>
                          <Package className="w-4 h-4 mr-2" /> Mark Fulfilled
                        </DropdownMenuItem>
                      )}
