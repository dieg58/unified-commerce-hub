...
export const RequireEmployee = ({ children }: { children: ReactNode }) => {
  ...
  if (!rolesLoaded) return <LoadingScreen />;
  if (isSuperAdmin) return <Navigate to="/dashboard" replace />;
  // Managers use their own layout with sidebar, so redirect them
  if (isShopManager || isDeptManager) return <Navigate to="/shop" replace />;
  if (isSubdomain && !profile?.tenant_id) {
    return <PendingApprovalScreen onSignOut={signOut} />;
  }
  return <>{children}</>;
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="text-center space-y-4 max-w-sm animate-fade-in">
        ...
        <h1 className="text-xl font-bold text-foreground">{t("auth.pendingApproval")}</h1>
        <p className="text-sm text-muted-foreground">{t("auth.pendingApprovalDesc")}</p>
        <Button variant="outline" onClick={onSignOut} className="mt-4">{t("auth.signOut")}</Button>
      </div>
    </div>
  );
};
